...
int num = 0;
int size;
string s, token, prev_token, token1, token2 = "";
...
void lexer(string input) {
	int i = 0;
	//remove emptys in the beginning
...
void expression() {
	if (set(fs_factor, 4) || isIdentifier || isInt || isReal) {
		...
		outFile << "syntax error: not an expression" << endl;
}
void Eprime() {
	if (token == "+")
	{
	...
	else if (token == ")" || token == ";"||token=="}"||token=="<"||token==">"||token=="!="||token=="="||token=="==")//follow E'=follow E
